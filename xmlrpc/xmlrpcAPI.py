...
_HOST = 'localhost'
#_HOST = '50.56.194.140'
_BASE_ADRESS = '/xmlrpc/ros_service'
_REFERENCE_PREFIX = 'REF:'

_SERVER = xmlrpclib.ServerProxy('http://{0}{1}'.format(_HOST, _BASE_ADRESS))
...
#class FileHandle(object):
    ...
    #def convertToTuple(self, fieldname):
        #return (fieldname, self.fileHandle)
...
def getAvailableServices():
    """ Get available nodes and services.
        ...
        @rtype:     { str : {} }
    """
    return json.loads(_SERVER.getAvailableServices())
...
def addEnv():
    """ Create a new environment.
        ...
        @rtype:     str
    """
    return json.loads(_SERVER.changeEnv(None, {}, []))['envID']
...
def changeEnv(envID=None, nodesToAdd=[], nodesToRemove=[]):
    ...
    # atm without support for files
    return json.loads(_SERVER.changeEnv(envID, data, ''))['envID']
...
def getEnv(envID):
    """ Get running nodes in environment.
        ...
        @rtype:     { str : str }
    """
    return json.loads(_SERVER.getEnv(envID))
...
def addTask(envID, service, message):
    ...
    data = json.dumps({'service' : service, 'msg' : msg })
    
    #atm without support for  files
    return json.loads(_SERVER.addTask(envID, None, body, ''))['taskID']
...
def getTask(envID, taskID):
    """ Get the status/result of a task.
        ...
        @rtype:     (str, {})
    """
    response = json.loads(_SERVER.getTask(envID, taskID))
...
def removeTask(envID, taskID):
    """ Remove a task.
        ...
        @type  taskID:      str
    """
    _SERVER.removeTask(envID, taskID)
