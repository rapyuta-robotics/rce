...
configuration/setup overheads, dependence on custom middleware, as well as maintenance and update overheads. 
	...
	\section*{RoboEarth App Engine Framework}
		The RoboEarth App Engine Framework allows users to upload, select, and use apps.
		The framework also acts as an interpreter between user
		requests and their apps. 
		It allows 
communication with processes using \textsc{ROS}, process management
		...
		The \emph{django Interface} of the framework communicates with the clients. 
		To include a broader audience the communication uses the standard \textsc{http} protocol.
		...
		implementation the focus is on the robots as users. 
	...
	\section*{Requirements}
		...
		To further
		distinguish and control the interaction dynamic URLs are used. This leads basically to
...
BASE/api/reappengine/[envID/[taskID/[fileRef/]]]
		...
		On the next level, \emph{Environment}, the user can create a task which is essentially the input message for
		the selected \textsc{ROS} \emph{service}. Again, he can get some information specific to
		his current environment.
		
		And on the last level, \emph{Task}, the user can retrieve the status or, if the task has been completed
		successfully, the result of the task. The result is basically the response message of the selected service.
		
		
		The forth level \emph{fileRef} is only used to provide
		the possibility to download referenced files for the tasks result, e.g. an image file. 
		...
		the interpretation of the responses a small Python-based ServiceAPI is provided. For a graphical
		representation of these interactions refer to figure \ref{fig:diagram}. 
		
		\begin{figure}
			\begin{center}
				\def\svgwidth{\columnwidth}
				%\input{frameworkOverview.pdf_tex}
				...
				necessary \emph{services} of the \emph{Server Node} or \emph{Environment Node}. The \emph{Environment Node} is created and managed by the \emph{Server Node}. The user selected \emph{App Nodes} are then launched and managed by
				the \emph{Environment Node} which also provides the necessary
				\emph{services} to create tasks and retrieve their results.}
				\label{fig:diagram}
			\end{center}
		\end{figure}
