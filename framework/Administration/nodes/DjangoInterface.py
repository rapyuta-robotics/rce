...
addTask, getTask, getFile, removeTask, \
addNode
...
# Custom imports
import settings
from MessageUtility import preprocessFiles, genericService, InvalidRequest, InternalError, FatalError
...
""" The django interface functions have the following constraints:
    ...
    @param ref:     File reference which can be found in data string.
    @type  ref:     str
...
def readTask(envID, taskID):
    ...
    return json.loads(genericService('{0}/getTask'.format(envID), getTask, taskID=taskID))

def readFile(envID, taskID, ref):
    """ This function is used when a 'GET' request on task level has to
        be processed.
    """
    try:
        rospy.wait_for_service('{0}/getFile'.format(envID), timeout=settings.WAIT_FOR_SERVICE_TIMEOUT)
    except rospy.ROSException:
        raise InvalidRequest('Could not find environment.')
    
    return genericService('{0}/getFile'.format(envID), getFile, taskID=taskID, ref=ref)
