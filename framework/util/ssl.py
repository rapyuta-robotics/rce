...
def createCertReq(key, name):
    """ Create a X509 certificate request.
        ...
        @param key:     Private key which should be used together with the
                        certificate.
...
def createCert(req, issuerCert, issuerKey, valid=365*24*60*60):
    """ Create a X509 certificate using a certificate request and a
        certificate/key to sign the newly created certificate.
        
        @param req:     Certificate request which should be processes and
                        signed.
        @type  req:     OpenSSL.crypto.X509Req
        
        @param issuerCert:  Certificate of the issuer, i.e. the certificate
                            which is used to sign the new certificate.
        @type  issuerCert:  OpenSSL.crypto.X509
        
        @param issuerKey:   Key of the issuer, i.e. the key matching the
                            certificate which is used to sign the new
                            certificate.
        @type  issuerKey:   OpenSSL.crypto.PKey
        
        @param valid:   Duration which the certificate will be valid in seconds
                        from now.
        @type  valid:   int
...
def createKeyCertPair(name, caCert=None, caKey=None):
    """ Create a key/certificate pair which can be either self-signed or signed
        ...
        @param caCert:  Certificate of the issuer, i.e. the certificate which
                        is used to sign the new certificate.
                        None means that the certificate is self-signed.
        @type  caCert:  OpenSSL.crypto.X509
        
        @param caKey:   Key of the issuer, i.e. the key matching the
                        certificate which is used to sign the new certificate.
                        None means that the certificate is self-signed.
        @type  caKey:   OpenSSL.crypto.PKey
        
        @return:        Tuple consisting of the pair certificate and key.
        @rtype:         (OpenSSL.crypto.X509, OpenSSL.crypto.PKey)
    """
    key = createKey()
...
def loadCertFile(fileName):
    """ Load a certificate from a file.
        ...
        @param fileName:    Path to the file from which the certificate should
                            be loaded.
        @type  fileName:    str
...
def loadKeyFile(fileName):
    """ Load a key from a file.
        ...
        @param fileName:    Path to the file from which the key should be
                            loaded.
        @type  fileName:    str
...
def writeCertToFile(cert, path):
    """ Write a certificate to a file.
        ...
        @type  req:     OpenSSL.crypto.X509
        
        @param path:    Path of the file where the Certificate should be
                        written.
        @type  path:    str
    """
...
def dumpCertReq(req):
    """ Dump a certificate request to string.
        ...
        @param req:     Certificate request which should be dumped into a
                        string.
        @type  req:     OpenSSL.crypto.X509Req
...
def parseCertStr(cert):
    """ Parse a certificate which is in string form.
        ...
        @param cert:    Certificate in string form, i.e. content of certificate
                        in file form.
        @type  cert:    str
...
class RCEServerContext(ContextFactory):
    ...
    def __init__(self, ca, cert, key):
        """ Initialize the RCEServerContext.
            ...
            @param ca:      Path to the certificate which has been used to sign
                            the certificate/key of this Context and which is
                            used to identify the other side.
            @type  ca:      str
            
            @param cert:    Either a certificate X509 instance or a path to the
                            certificate of this Context.
            @type  cert:    OpenSSL.crypto.X509 / str
            
            @param key:     Either a key instance or a path to the key of this
                            Context.
            @type  key:     OpenSSL.crypto.PKey / str
        """
