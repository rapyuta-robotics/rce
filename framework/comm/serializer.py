...
class Serializer(object):
    ...
    def addHeader(self, length, dest, origin, msgType, msgNr):
        ...
        """
        targetLength = len(self._buf) + definition.HEADER_LENGTH
        
        if targetLength != length:
            raise SerializationError('Expected length of serializer '
                                     'does not match given message length.')
        ...
        if len(self._buf) != targetLength:
            raise SerializationError('Length of header does not match '
                                     'the specifications.')
    ...
    def addIdentifier(self, identifier, length):
        """ Serialize a single identifier.
            ...
            @param length:  Length of the identifier.
                            The length should be predefined as it is only used
                            to make sure that the correct number of bytes are
                            written. The information itself is not added to the
                            message.
            @type  length:  int
            
            @raise:     errors.SerializaitonError if the length of the
                        identifier does not match the given length.
        """
        if len(identifier) != length:
            raise SerializationError('Length of the identifier does not match '
                                     'the specification.')
    ...
    def addElement(self, element):
        """ Serialize a single element.
            ...
            @raise:     TypeError if the element is not of type str
                        or of type None.
        """
        if element is None:
            ...
            try:
                ...
                raise SerializationError('The string "{0}" can only contain '
                                         'ASCII chars.'.format(element))
    ...
    def addDict(self, data):
        ...
        for (key, value) in data.iteritems():
            try:
                ...
                raise SerializationError('Key is neither a of type str '
                                         'nor of type None.')
            
            try:
                self.addElement(value)
            except TypeError:
                raise SerializationError('Value is neither a of type str '
                                         'nor of type None.')
    ...
    def addFIFO(self, fifo):
        """ Add the serialized data of the FIFO to the current FIFO.
            ...
            @param data:    FIFO containing serialized data which should be
                            added.
...
class Deserializer(object):
    ...
    def getIdentifier(self, length):
        """ Serialize a single identifier.
            ...
            @param length:  Length of the identifier.
                            The length should be predefined as it is only used
                            to know how many bytes should be read.
            @type  length:  int
    ...
    def getDict(self):
        ...
        """
        return dict((self.getElement(), self.getElement())
                        for _ in xrange(self.getInt()))
    ...
    def getList(self):
        """ Deserialize the data.
            ...
            @raise:     SerializationError if the format does not match that of
                        a serialized list.
        """
